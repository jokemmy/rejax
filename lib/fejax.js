...
var isAjax = (0, _whatitis.property)(ajaxSymbol);
var globalMap = {
  ...
  setURLAlias: function setURLAlias(aliases, transform) {
    ...
    if (_whatitis2['default'].Function(transform)) {
  ...
  setAssert: function setAssert(assert) {
    if (_whatitis2['default'].Function(assert)) {
...
var allocator = function allocator(resolve, reject, options) {
  return function (success) {
    return function (cdResponse, xhr) {
      ...
      if (success) {
        if (_whatitis2['default'].Function(options.assert)) {
...
function getOptions(url, options_) {
  ...
  if (_whatitis2['default'].String(url)) {
    options.url = url;
    if (_whatitis2['default'].PlainObject(options_)) {
      assignOption(options, options_);
    }
    options.method = getMethod(options);

    // url is PlainObject, only one argument
  } else if (_whatitis2['default'].PlainObject(url)) {
    assignOption(options, url);
    options.method = getMethod(options);

    // url is Function, only one argument
  } else if (_whatitis2['default'].Function(url)) {
    options[ajaxHandler] = url;
  }

  // dataType default 'json'
  if (!_whatitis2['default'].String(options.dataType) || options.dataType === '') {
    options.dataType = 'json';
  }

  if (_whatitis2['default'].String(options.url)) {
    options.url = URLFormat(options.url);
  }

  // null/undefined => String('')
  if (_whatitis2['default'].PlainObject(options.data)) {
    options.data = Object.entries(options.data).reduce(function (memo, _ref8) {
      var _ref9 = _slicedToArray(_ref8, 2),
          key = _ref9[0],
          value = _ref9[1];

      memo[key] = _whatitis2['default'].Defined(value) ? value : '';
      return memo;
    }, {});
  }
...
function composeWithResult(callback, lastHandle) {
  return function () {
    ...
    if (_whatitis2['default'].Defined(result)) {
...
function Ajax(url, options) {
  ...
  function newAjax(options) {
    var abort = void 0;
    var chain = _chain2['default'].of(function (resolve, reject) {
      var callback = allocator(resolve, reject, options);
      abort = (0, _xhr2['default'])(options).then(callback(true))['catch'](callback(false)).abort;
    });
    chain.abort = abort;
    return chain;
  }
  ...
  function append(funcName) {
    ...
    var nextArgs = args.length === 1 && _whatitis2['default'].Function(args[0]) && needLastResults ? function (result) {
      var _args$;
  ...
  function setXhrs() {
    return builders.reduce(function (xhrs, options) {
      ...
      if (_whatitis2['default'].Function(options)) {
        var ajax = options();
        (0, _invariant2['default'])(isAjax(ajax), 'Hope: Function of ajax() expecting a ajax-object be returned.');
  ...
  function getNext(ajaxObject) {
    (0, _invariant2['default'])(isAjax(ajaxObject), 'Hope: Function of ajax() expecting a ajax-object be returned.');
  ...
  function connection() {
    ...
    sending = _chain2['default'].all(xhrs).then(function () {
      var result = void 0;
      if (handlerThen) {
  ...
  ['ajax', 'get', 'post', 'put', 'delete'].forEach(function (method) {
    chainMethod[method] = (0, _compose2['default'])(function (options) {
      if (createAjax(options, method === 'ajax' ? options.method : method)) {
        return chainMethod;
      }
      return (0, _object2['default'])(chainMethod, ['ajax', 'get', 'post', 'put', 'delete']);
...
Object.assign(Ajax, (0, _object2['default'])(globalMap, ['setURLTokens', 'setURLAlias', 'setDataTransform', 'setAssert']), {
  getURL: URLFormat
});
